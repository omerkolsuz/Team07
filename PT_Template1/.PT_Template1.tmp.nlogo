...
citizens-own [
  state ;läge för citizens
  home-x
  home-y
  standing-time ; tiden för dem stannar efter de hittaar andra citizen för demonstrationen
...
to go
  ...
  ask turtles [
   ...
   ; if state = "go-to-square" [ go-to-town-square ]
   ; if state = "demonstration" [ go-to-check-others ]
    ;if state = "standing-still" [ go-home ]
    ;if state = " go-home " [ ] ; här vet vi inte vad vi ska lägga in
  ]
...
to go-to-town-square
  let destination one-of patches with [region = "locTownSquare"]
  face destination
end

;walk and look around to find other citizens who also want to demonstrate
to go-to-check-others
  if any? other citizens-here with [state = "demonstration"][
  set state "standing-still"
  set standing-time 60 ; en timme standing time
]
end

;stand still once they find someone else, for about 1 hr
to standing-still
  if standing-time > 0 [
...
to update-time-flags
  ...
  set flagMorning (hour-of-day >= 6 and hour-of-day < 10) ;morgonen från kl 06.00 till 10.00
  set flagAfternoon (hour-of-day >= 10 and hour-of-day < 16) ;morgonen från kl 06.00 till 10.00
  set flagEvening (hour-of-day >= 16 and hour-of-day < 22) ; kväll från 16.00 till 22.00
  set flagWeekend (time >= 120 and time mod 168 < 48)
end



to execute-agent-behaviors
  ask turtles [
    if breed = citizens [
      ; Define behaviors for citizens
    ]
    if breed = cops [
      ; Define behaviors for cops
    ]
  ]
end
...
; ########################## AGENT BEHAVIORS ##########################
;to citizen_behavior
  ; Detailed behaviors for citizen agents
;end

;to cop_behavior
  ; Detailed behaviors for cop agents
;end
@#$#@#$#@
GRAPHICS-WINDOW
549
